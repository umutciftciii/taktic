  ...
  if (user.role === 'CUSTOMER') {
    return (
      <nav className="lp-nav" aria-label="Site navigation">
        <Link href="/categories">Kategoriler</Link>
        <Link href="/requests/my">Taleplerim</Link>
        <Link href="/categories">Yeni Talep Oluştur</Link>
      </nav>
    );
  }
  ...
  if (user.role === 'PROVIDER') {
    return (
      <nav className="lp-nav" aria-label="Site navigation">
        ...
        {providerId ? <Link href={`/providers/${providerId}/requests`}>İşler</Link> : null}
...
function HeaderCta({ user }: { user: AuthUser | null }) {
  ...
  return (
    <div className="lp-header-cta">
      {user.role === 'CUSTOMER' ? (
        <Link className="btn btn-primary btn-sm lp-cta-mobile-hide" href="/categories">
          Yeni Talep Oluştur
        </Link>
      ) : null}
      <UserMenu user={user} />
  ...
  return (
    <details className="lp-user">
      <summary className="lp-user-summary" aria-label="Kullanıcı menüsü">
        ...
        <span className="lp-user-caret" aria-hidden="true">▾</span>
      </summary>
