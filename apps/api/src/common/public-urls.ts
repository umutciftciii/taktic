 ...
 * Two rules:
 *
 * 1. **Production must say it.** There is no fallback there. A deployment that
 *    does not declare its own address would mail links to `localhost`, and the
 *    failure would only surface in somebody else's inbox.
 ...
 *    for development, and that exemption is what the production rule closes.
 *
...
/** The development fallback. Never used when NODE_ENV=production. */
...
export function readPublicWebBaseUrl(): string {
  ...
  if (!raw) {
    if (isProduction()) {
      throw new Error(
        'WEB_APP_URL is required in production: every link this application mails is built from ' +
          'it, and there is no address it could guess that would not be wrong.',
      );
    }
...
function normalizeBaseUrl(raw: string, name: string): string {
  ...
  if (isProduction() && isLoopback(parsed.hostname)) {
    throw new Error(
      `${name} must not point at loopback in production: the links built from it are opened from ` +
