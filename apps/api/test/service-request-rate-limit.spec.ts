...
describe('POST /service-requests rate limits', () => {
  ...
  it('answers 429 REQUEST_RATE_LIMITED once a phone has 10 open APPROVED requests', async () => {
    const category = await createCategory(ctx.prisma);
    const phone = '05553330001';

    for (let index = 0; index < SERVICE_REQUEST_MAX_OPEN_PER_PHONE; index += 1) {
      await createApprovedRequest(ctx.prisma, { categoryId: category.id, customerPhone: phone });
    }
