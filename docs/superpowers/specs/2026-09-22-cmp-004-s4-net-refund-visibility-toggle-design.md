...
| D7 | **Sağlayıcı promo görünürlüğü:** `GET /providers/:providerId/credits` yanıtına `promo: { spendableCredits, lots: [{ id, remainingCredits, expiresAt, campaignName }] }` eklenir. Lot seçimi S2B1 ile aynı predicate: `status = ACTIVE ∧ remainingCredits > 0 ∧ expiresAt > now`, `PROMO_SPENDABLE_LOT_ORDER` (en yakın son kullanma önce). Kampanya adı `redemption.campaign.name`; kural/koşul/limit/başka sağlayıcı verisi yok. Guard mevcut `ProviderAccessGuard` (kendi sağlayıcısı ya da SUPER_ADMIN). Web kredi sayfasında "Promosyon kredileri" bloğu yalnız en az bir kullanılabilir lot varken render edilir; toplam + lot listesi (kredi, tarih, kampanya). | Görev tanımı C. Ayrı endpoint yerine mevcut yanıt: sayfa zaten bu uç noktayı okuyor, guard'ı test edilmiş. Süresi geçmiş/revoke lotlar predicate dışı → toplama girmez. |
...
- **Sağlayıcı kredileri:** `getProviderCredits` → mevcut bakiye + son 20 işlem + `promo` (lot listesi).
...
| 5 | `GET /providers/:id/credits.promo`: kendi lotları (sıra, toplam), expired/revoked/exhausted dışarıda; anon 401, CUSTOMER/başka PROVIDER 403; kampanya kural verisi yok | `apps/api/test/provider-promo-visibility.spec.ts` (yeni) |
